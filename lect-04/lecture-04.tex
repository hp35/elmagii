...
v{\aa}gutbredning med Maxwell's ekvationer (med assistans av en till{\"a}ggsterm
till Amp\`eres magnetostatiska lag, som vi d{\aa} kommer att g{\aa} igenom).
{\it Notera h{\"a}r hur vi genomg{\aa}ende kan sp{\aa}ra f{\"o}rekomsten av
$\mu_0$ till Biot--Savarts lag.}
...
$$
  \iint_{S}(\nabla\times{\bf B})\cdot d{\bf A}=
  \oint_{\Gamma}{\bf B}\cdot d{\bf l}=
  \mu_0\iint_{S}{\bf J}\cdot d{\bf A},
$$
d{\"a}r nu $\iint_{S}{\bf J}\cdot d{\bf A}=I_{\rm enc}$ {\"a}r den av $\Gamma$
...
$$
Denna form {\"a}r ofta att f{\"o}redra i situationer d{\aa} vi kan utnyttja rent
geometriskt--symmetriskt gynn\-samma situationer, p{\aa} precis samma s{\"a}tt
som vi kunnat konstatera med Gauss lag och hur den markant kan f{\"o}renkla
probleml{\"o}sande i elektrostatiska problem med symmetrier n{\"a}rvarande.
...
\item{$\bullet$}{Biot--Savarts generella lag f{\"o}r samband mellan
  ...
  lag f{\"o}r str{\"o}mslingor med str{\"o}m $I$ ges som
  $$
    {\bf B}({\bf x})={{\mu_0}\over{4\pi}}\int^{{\bf x}_b}_{{\bf x}_a}
        {{{\bf I}({\bf x}')\times({\bf x}-{\bf x}')}
          \over{|{\bf x}-{\bf x}'|^3}}\,dl'.
...
\item{$\bullet$}{Amp\`eres lag f{\"o}r vektorpotentialen ${\bf A}$ (andra
  ekvationen p{\aa} omslaget p{\aa} Griffiths!),
  $$
    \nabla^2{\bf A}=-\mu_0{\bf J}.
  $$}
